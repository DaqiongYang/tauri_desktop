...
import { onMounted, onUnmounted, computed, nextTick, ref, shallowRef, watch, type ComputedRef, type PropType, type Ref, type ShallowRef, type WritableComputedRef } from 'vue';
...
const mouseleave = () => {
  if (view.value) {
    position.value = getCursor();
    document.querySelector('.cm-content')?.removeAttribute('contenteditable');
  }
};
const mouseenter = () => {
  if (view.value) {
...
// 定义事件传播
const emits = defineEmits(['update:value', 'update', 'ready', 'focus', 'blur', 'change', 'destroy']);
...
// 文本值
const doc: Ref<string | Text> = ref(props.value) as any;
// 编辑器
const view: ShallowRef<any> = shallowRef(new EditorView());
...
const selection: WritableComputedRef<any> = computed({
...
const json: WritableComputedRef<Record<string, any>> = computed({
...
const extensions: ComputedRef<any[]> = computed(() => {
  ...
  return [
    ...
    ...props.extensions,
  ].filter((extension): extension is any => !!extension);
});
watch(
  extensions,
  (exts) => {
...
onMounted(async () => {
  ...
  view.value = new EditorView({
    ...
    dispatch: (tr: any) => {
      ...
      emits('update:value', (tr.state.doc as any).toString());
      emits('change', tr.state);
    },
  });
  /*
  // view.value.dispatch('s', () => {

  // })
  let a = document.createElement('button');
  let t = document.createTextNode('立即执行');
  a.addEventListener(
    'click',
    () => {
      alert(111);
  ...
  a.appendChild(t);
  codemirror.value.appendChild(a);
 */
  await nextTick();
  emits('ready', {
    view: view.value,
...
// 选中数量
const listSelections = (): readonly any[] => {
  let val;
...
// 设置选中多个
const setSelections = (ranges: readonly any[], primary?: number): void =>
  view.value.dispatch({
...
const extendSelectionsBy = (f: any): void =>
  view.value.dispatch({
    selection: EditorSelection.create(selection.value.ranges.map((r: any) => r.extend(f(r)))),
  });
